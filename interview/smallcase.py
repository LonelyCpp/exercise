...
# pattern starting at a location marked 1 and then
# counting up while spiraling outward. For example,
...
# from the square identified in your puzzle input
# all the way to the access port?


def get_layer(element):
    layer = 1
    while (layer * 2 - 1) ** 2 < element:
        layer += 1

    return layer


def get_distance(layer, element):
    if element == 1:
        return 0

    elements_in_row = layer * 2 - 1
    max_elem = elements_in_row ** 2

    diff = max_elem - element

    dist_from_corner = diff % (elements_in_row - 1)
    dist_from_middle = abs(elements_in_row // 2 - dist_from_corner)

    dist_to_layer = layer - 1

    return dist_from_middle + dist_to_layer


def compute_distance(elem):
    layer = get_layer(elem)
    print(get_distance(layer, elem))
